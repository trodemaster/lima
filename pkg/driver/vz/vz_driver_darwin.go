...
func (l *LimaVzDriver) Stop(ctx context.Context) error {
	...
	if l.machine.CanRequestStop() {
		// The returned boolean tells whether the request was made, not whether the guest
		// has already stopped.
		var err error
		if requested, err = l.machine.RequestStop(); err != nil {
			logrus.WithError(err).Warn("vz: failed to request the guest to stop")
		}
	} else {
		logrus.Warn("vz: CanRequestStop is not supported")
	}
